...
+function ($) {
    ...
    Datagrid.DEFAULTS = {
        ...
        editPage: ''      // Function - after delete method
    }
        ...
        var tools = {
            ...
            columns2Arr: function (columns, rowArr, index, parent) {
                ...
                $.each(columns, function (i, n) {
                    ...
                    if (n.columns) {
                        ...
                        n.editHide = (typeof n.editHide == 'undefined') ? false : n.editHide
            ...
            initTbody: function (data, refreshFlag) {
                ...
                var doInit = function () {
                    ...
                    if (!allData.length) {
                        ...
                        if (options.paging) { //吴秀桐添加判断如果分页组件启用才进行分页操作 20150821
                        ...
                        } else {//吴秀桐添加判断如果分页组件未启用，把开始和结束重新赋值 20150821
                            start = 0;
                            end = allData.length
                        }
                    }
            ...
            createTrs: function (datas, refreshFlag) {
                ...
                $.each(datas, function (i, trData) {
                    ...
                    //吴秀桐。判断如果没有启用分页则直接将序号赋值非行序号
                    if(!options.paging || options.paging=="false")linenumber=i+1;
                    ...
                    $.each(model, function (j, n) {
                        ...
                        /* append */
                        $td = $('<td name="' + name + '"' + align + cls + '><div>' + label + '</div></td>')
                        ...
                        if (n.calc) {
                            ...
                            //alert("name:"+name+",number:"+number+","+n.calc_sum+"：412")
                        }
                    })
                ...
                if (refreshFlag) {
                    ...
                    //吴秀桐20150905添加else解决如果没有返回数据，则直接下面的统计行赋值为0
                    if (datas.length <= 0) {
                        $.each(that.columnModel, function (j, n) {
                            //吴秀桐 修改刷新时候合计数据累计的问题！
                            /* for tfoot */
                            if (n.calc) {
                                if (n.calc == 'sum' || n.calc == 'avg') n.calc_sum = 0
                                else if (n.calc == 'max') n.calc_max = 0
                                else if (n.calc == 'min') n.calc_min = 0
                            }
                        })
                    }
                    that.initTfoot();//吴秀桐添加用于解决刷新按钮不刷新统计行数据的bug
                }
                        ...
                        if (op.type) {
                            switch (op.type) {
                                case 'date':
                                    ...
                                    that.inputs.push('<input type="text"  onlybtn=true name="' + name + '" data-toggle="datepicker"' + pattern + rule + attrs + '>')
                                ...
                                case 'lookup':
                                    ...
                                    //吴秀桐修改 20150606原因在于当有这个属性时无法lookup多个
        ...
        if ($trs.length && trLen == 1) {
            ...
            $trs.find('> th').each(function (i) {
                ...
                if (that.columnModel.length && that.columnModel[i]) {
                    ...
                    op.editHide = (typeof op.editHide == 'undefined') ? false : op.editHide
    ...
    //吴秀桐增加用于提供url刷新！
    Datagrid.prototype.refreshByUrl = function (dataUrl) {
                            ...
                            that.$toolbar_edit = $(btnHtml).attr('data-icon', 'edit').addClass('btn-green').text(BJUI.getRegional('datagrid.edit'))
                                ...
                                .on('click', function (e) {
                                    ...
                                    if (!$selectTrs.length) {
                                        ...
                                        //2015年0706吴秀桐修改增加判断自定义跳转编辑界面还是使用原生的编辑
                                        if (options.editPage.length > 0) {
                                            var Pk = '';
                                            var columns_1 = options.columns;
                                            var index_ = -1;
                                            $.each(columns_1, function (i, n) {
                                                if (n.name == options.delPK) {
                                                    index_ = i;
                                                    var tds = $selectTrs.first().find("td");
                                                    Pk = $(tds[i]).text();
                                                    return false;
                                                }
                                            });
                                            // alert(Pk+":"+options.editPage);
                                            if (options.editMode == 'dialog') {
                                                $(this).dialog({
                                                    id: 'alter' + $(this).attr("id"),
                                                    url: options.editPage + "?khdm=" + Pk,
                                                    title: "修改" + options.gridTitle
                                                });
                                            } else {
                                                $(this).navtab({
                                                    id: 'alter' + $(this).attr("id"),
    ...
    Datagrid.prototype.initThead = function () {
        ...
        var delayFunc = function () {
            if (options.showTfoot) {
                //alert("initThead 调用initTfoot");
                that.initTfoot()
            }
            that.initLock()
            that.$grid.initui()
    ...
    Datagrid.prototype.colMenu = function () {
        ...
        $ths.each(function () {
            ...
            if (!$btnBox.length) {
                $btn = $('<button class="btn btn-default" type = button><i class="fa fa-bars"></button>'),
    ...
    Datagrid.prototype.dialogEdit = function ($tr, isAdd, data) {
        ...
        $form = $('<form class="datagrid-dialog-edit-form" data-validator-option="{timely:false}"></form>')
        ...
        var onChange = function ($tr, $form, $el, model) {
            ...
            switch (model.type) {
                ...
                case 'boolean':
                    ...
                    break
                case 'lookup':
                    $form.on('afterchange.bjui.lookup', function (e, args) {
                        //$form.on('customEvent.bjui.lookup', '[data-toggle="lookupbtn"]', function(e, args) {

                        //for (var key1 in name1) {
                        //}
                        for (var key in args) {
                            var param = args[key].substring(0, args[key].indexOf(":"));
                            var value = args[key].substring(args[key].indexOf(":") + 1, args[key].length);

                            //alert("changeData[key]:"+changeData[key]+";data[key]:"+data[key]+";args-key:"+key+";model.name :"+model.name );
                            // if (typeof data[key] != 'undefined') {
                            ...
                            if (model.name == key) {
                                ...
                                //  alert("key:"+json.key+";value:"+args[key]);
                            }
                        }
                    });
                    $el.change(function () {
        ...
        var onLoad = function ($dialog) {
            ...
            var createForm = function (data, $form, $tr) {
                ...
                $.each(columnModel, function (i, n) {
                    //判断该列是否需要显示！
                    var editHide = (typeof  n['editHide'] == 'undefined') ? false : n['editHide']
                    if (n == that.linenumberColumn || n == that.checkboxColumn || n == that.editBtnsColumn) return true
                    ...
                    var id = 'datagrid-dialog-edit-column-' + i
...
//dialog修改按钮事件
                        ...
                        if (callback) {
                            ...
                            callback = function (json) {
                                ...
                                if (complete) {
                                    ...
                                    //吴秀桐，修改为点击保存后不退出界面
                                    if(options.editUrl== 'ajaxDone1.html' || !options.editUrl || options.editUrl=='') {
    ...
    Datagrid.prototype.resizeGrid = function () {
        ...
        var _resizeGrid = function () {
            ...
            if (String(that.options.height).endsWith('%') || String(that.options.height)==null) {
    ...
    /* 吴秀桐增加自动刷新表格大小20150919 resize */
    Datagrid.prototype.resizeGridByAuto = function () {
        var that = this, $target = that.$grid.getPageTarget(), parentW, parentH
        parentW = that.$grid.parent().width();
        parentH = that.$grid.parent().height();
        that.fixedWidth()
